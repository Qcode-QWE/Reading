...
def get_calibration(image, max_num):
    ...
    scipy.misc.imsave('D:/USER/Desktop/软件工程/01灰度化.jpg',gray_img)
    # 对比度增强滤波。求出局部区域的最大值和最小值，然后看当前点像素值最接近最大值还是最小值，然后替换为最大值或最小值。
    median_img=sfr.median(gray_img,disk(3))
    scipy.misc.imsave('D:/USER/Desktop/软件工程/02滤波.jpg',median_img)
    # sobel 边缘检测
    edges_img = filters.sobel(median_img)
    scipy.misc.imsave('D:/USER/Desktop/软件工程/03边缘检测.jpg',edges_img)
    # 二值化处理
    thresh = filters.threshold_yen(edges_img)
    bin_img =(edges_img <= thresh)*1.0
    scipy.misc.imsave('D:/USER/Desktop/软件工程/04二值化.jpg',bin_img)
    bin_img_2 = 1- bin_img
    scipy.misc.imsave('D:/USER/Desktop/软件工程/04二值化01.jpg',bin_img_2)
    # 对连通区域进行操作
    # 删除小块区域
    ...
    scipy.misc.imsave('D:/USER/Desktop/软件工程/05连通区域-大-颜色.jpg', label_image)
    # 截取水尺图片
    roi_img = ml.get_roi_img(label_image, img)
    scipy.misc.imsave('D:/USER/Desktop/软件工程/06截取水尺.jpg',roi_img)
    # 找到旋转角度
    angle = ml.get_angle(label_image)
    # 灰度化
    gray_img = color.rgb2gray(roi_img)
    scipy.misc.imsave('D:/USER/Desktop/软件工程/07灰度化.jpg',gray_img)
    # 水平边缘检测
    edges_img=filters.sobel_h(gray_img)
    scipy.misc.imsave('D:/USER/Desktop/软件工程/08水平检测.jpg',edges_img)
    # 二值化
    thresh = filters.threshold_yen(edges_img)
    bin_img =(edges_img <= thresh)*1.0
    ...
    scipy.misc.imsave('D:/USER/Desktop/软件工程/12旋转.jpg',angle_img)
    # 提取所有的直线
    label_image = measure.label(label_image)
    dst = color.label2rgb(label_image)
    scipy.misc.imsave('D:/USER/Desktop/软件工程/13直线.jpg',dst)
    # 得到右侧线段的距离
